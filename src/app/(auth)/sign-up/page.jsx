...
const SignUpPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!/(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{8,}/.test(formData.password)) {
      toast.error("Password too weak", {
        description: "Must include uppercase, number, and special character (min 8 chars)",
        position: "top-center",
        duration: 5000,
      });
      return;
    ...
    try {
      ...
      toast.success("Account created!", {
        description: `Welcome ${data.user.name}! Signing you in...`,
        position: "top-center",
        duration: 3000,
      });
      ...
      if (result?.error) {
        throw new Error("Failed to sign in automatically: " + result.error);
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 p-4 animate-in fade-in">
      <div className="w-full max-w-md bg-white/90 backdrop-blur-sm rounded-xl shadow-lg p-6 md:p-8 border border-gray-200/50">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <input
              ...
              className="mt-1 block w-full border border-gray-300 rounded-lg py-2 px-3 text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-transparent transition-all"
          ...
          <button
            ...
            className="w-full bg-gray-900 hover:bg-gray-800 text-white py-2 md:py-3 px-4 rounded-lg transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm md:text-base"
          >
            {loading ? (
              <>
                <svg
        ...
        <div className="grid grid-cols-2 gap-2 md:gap-3">
          ...
          <button
            onClick={() => {
              toast.promise(signIn("github", { callbackUrl: "/" }), {
        ...
        <p className="text-center text-gray-600 mt-4 md:mt-6 text-xs md:text-sm">
